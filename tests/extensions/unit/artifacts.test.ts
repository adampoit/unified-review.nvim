...
test('artifact guards require the versioned bridge contracts', () => {
	...
	assert.equal(isSelectionArtifact({ schema: 'unified-review.agent-selection.v0' }), false);
	assert.equal(
		isContextArtifact({
	...
	assert.equal(isContextArtifact({ schema: 'unified-review.agent-context.v1' }), false);
});
