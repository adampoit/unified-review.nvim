...
import { registerDiffReviewExtension, type DiffReviewDependencies } from '../../../extensions/diff-review.ts';
...
function harness(options: { mode?: string; review?: string } = {}) {
	...
	const dependencies: DiffReviewDependencies = {
		...
		fileExists: (path) => path.endsWith('review.md') && options.review !== undefined,
		readJsonIfExists: (() => ({
			status: 'saved',
			thread_count: 1,
			exported_thread_count: 1,
		})) as DiffReviewDependencies['readJsonIfExists'],
		readTextIfExists: (path) => (path.endsWith('review.md') ? options.review : undefined),
...
test('/review rejects RPC mode before attempting a terminal handoff', async () => {
	...
	assert.deepEqual(instance.notifications, [['/review requires the interactive TUI', 'error']]);
});
...
test('/review inserts exported feedback and cleans its workspace', async () => {
	...
	assert.match(instance.writes.get('/tmp/diff-review-test/review-init.lua') ?? '', /summary\.save_active/);
	assert.deepEqual(instance.removed, ['/tmp/diff-review-test']);
	assert.deepEqual(instance.notifications.at(-1), ['Inserted Neovim review into the editor.', 'info']);
});
