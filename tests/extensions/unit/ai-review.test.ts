...
import { registerAiReviewExtension, type AiReviewDependencies } from '../../../extensions/ai-review.ts';
...
function harness(
	...
	const dependencies: AiReviewDependencies = {
		...
		runHeadlessNvim: async () => options.headlessResult ?? { exitCode: 0, stderr: '' },
...
test('/ai-review exports context, queues review, imports feedback, and settles', async () => {
	...
	const contextInit = instance.writes.get('/tmp/ai-review-test/context-init.lua') ?? '';
	...
	assert.match(instance.writes.get('/tmp/ai-review-test/feedback.json') ?? '', /test\/reviewer/);
	assert.match(instance.writes.get('/tmp/ai-review-test/import-init.lua') ?? '', /import_file/);
...
test('/ai-review reports context export diagnostics', async () => {
	...
	assert.match(instance.notifications[0][0], /message: failed to resolve review target/);
...
test('/ai-review requires true TUI mode', async () => {
	...
	assert.deepEqual(instance.notifications, [['/ai-review requires the interactive TUI', 'error']]);
});
