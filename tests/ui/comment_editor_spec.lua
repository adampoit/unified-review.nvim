...
local function temp_session()
	...
	return {
		...
		threads = {},
		ui = { files_buffer = files_buf, left_buffer = left_buf, right_buffer = right_buf },
	}
end
...
describe("comment editor", function()
	...
	it("saves a new comment from the scratch buffer", function()
		...
		local editor = comment_editor.open({ target = { kind = "file", path = "a.lua" } })
	...
	it("does not switch buffers inside BufWriteCmd when saving with :wqa", function()
		...
		vim.fn.writefile({
			...
			"local session = { id = 'child-session', target = { root = session_root }, selection = { file_index = 1 }, files = { { path = 'a.lua', status = 'modified', hunks = {} } }, threads = {}, ui = {} }",
			"state.set_active(session)",
	...
	it("wraps long lines in a stable multi-line popup window", function()
		local session = temp_session()
		state.set_active(session)
		local editor = comment_editor.open({ target = { kind = "file", path = "a.lua" } })
		...
		assert.are.equal(height, vim.api.nvim_win_get_height(win))
