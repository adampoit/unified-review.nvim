...
function terminalHtml(terminal: ArtifactTerminal) {
	...
	for (let y = buffer.baseY; y < buffer.length; y++) {
		...
		for (let x = 0; x < term.cols; x++) {
			...
			cells.push(style ? `<span style="${style}">${escapeHtml(chars)}</span>` : escapeHtml(chars));
		}
...
export function writeArtifactIndex(options: { title?: string; filename?: string; prefix?: string } = {}) {
	if (!enabled) {
		return;
	}
	...
	const links = files
		.map((file) => {
			...
			const txt = `${stem}.txt`;
			const formats = [];
			if (existsSync(join(root, svg))) formats.push(`<a href="${escapeHtml(svg)}">svg</a>`);
			if (existsSync(join(root, txt))) formats.push(`<a href="${escapeHtml(txt)}">txt</a>`);
			return `<li>
