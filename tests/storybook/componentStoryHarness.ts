...
export function useComponentStorybookTerminal(options: { columns?: number; rows?: number } = {}) {
	test.use({
...
export async function openComponentStory(terminal: Terminal, story: string) {
	...
	terminal.write(`:lua require('storybook.component_storybook').open('${story}')\r`);
}

export async function expectComponentStory(terminal: Terminal, story: string, title: string) {
