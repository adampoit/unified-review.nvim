...
import { configureNvimTest, createRepoFromDiffScenario, delay, vimEscapePath } from './helpers.js';
...
function createWorkingTreeRepo() {
	return createRepoFromDiffScenario(
		diffScenario([
			file('src/picker.txt', [del('old', ['PICKER_BASE_VALUE']), add('target', ['PICKER_WORKING_TARGET'])]),
		]),
...
test('UnifiedReview opens the target picker and selects working-tree changes', async ({ terminal }) => {
	const repo = createWorkingTreeRepo();
	await cdIntoRepo(terminal, repo);

	terminal.write(':UnifiedReview\r');
	await expect(terminal.getByText('Unified Review', { strict: false })).toBeVisible();
	await expect(terminal.getByText('Working tree changes', { strict: false })).toBeVisible();
	...
	terminal.write('\r');
	await expect(terminal.getByText('PICKER_WORKING', { strict: false })).toBeVisible();
});

test('commit range picker shows inline validation and then opens a valid range', async ({ terminal }) => {
	...
	terminal.write('b\r');
	await expect(terminal.getByText('Base and head must be different', { strict: false })).toBeVisible();
	captureTerminal(terminal, 'target picker - commit range validation');

	terminal.write('jb\r');
	await expect(terminal.getByText(/Loaded [0-9]+ changed file/g)).toBeVisible();
	terminal.write('\r');
	await expect(terminal.getByText('PICKER_COMMIT_RANGE', { strict: false })).toBeVisible();
});
