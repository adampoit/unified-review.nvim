...
import { add, ctx, del, diffScenario, file, hunk, scenarioFiles } from '../diffDsl.js';
import { configureNvimTest, createRepoWithFiles } from '../helpers.js';
import { luaString, openReviewForRepo, runLua, waitForBuffer } from './helpers.js';
...
const cases = [
	{
		...
		scenario: diffScenario([
			file('src/addition-between-context.txt', [ctx('before', 2), add('target', 2), ctx('after', 2)]),
		]),
	...
	},
	{
		...
		scenario: diffScenario([
			file('src/deletion-between-context.txt', [ctx('before', 2), del('target', 2), ctx('after', 2)]),
		]),
		anchorLabel: 'target:1',
	},
	{
		...
		scenario: diffScenario([
			file('src/equal-replacement.txt', [ctx('before', 2), del('old', 2), add('new', 2), ctx('after', 2)]),
		]),
		anchorLabel: 'new:1',
	},
	{
		...
		scenario: diffScenario([
			file('src/right-longer-replacement.txt', [ctx('before', 2), del('old', 1), add('new', 3), ctx('after', 2)]),
		]),
		anchorLabel: 'new:2',
	},
	{
		...
		scenario: diffScenario([
			file('src/left-longer-replacement.txt', [ctx('before', 2), del('old', 3), add('new', 1), ctx('after', 2)]),
		]),
		anchorLabel: 'old:2',
	},
	{
	...
	},
	{
		...
		scenario: diffScenario([file('src/start-of-file-addition.txt', [add('target', 2), ctx('after', 3)])]),
		anchorLabel: 'target:1',
	},
	{
		name: 'end-of-file-addition',
		scenario: diffScenario([file('src/end-of-file-addition.txt', [ctx('before', 3), add('target', 2)])]),
		anchorLabel: 'target:1',
	},
	{
		name: 'end-of-file-deletion',
		scenario: diffScenario([file('src/end-of-file-deletion.txt', [ctx('before', 3), del('target', 2)])]),
		anchorLabel: 'target:1',
	},
	{
		...
		scenario: diffScenario([
			file('src/multi-hunk-change.txt', [
				hunk([ctx('first_before', 2), del('first_old', 1), add('first_new', 1), ctx('first_after', 2)]),
				hunk([ctx('second_before', 2), add('second_new', 2), ctx('second_after', 2)], { gapBefore: 8 }),
			]),
		]),
	...
	},
	{
		...
		scenario: diffScenario([
			file('src/two-file-inline.txt', [ctx('inline_before', 2), add('inline_target', 1), ctx('inline_after', 2)]),
...
test('renders deterministic diff-shape scenarios', async ({ terminal }) => {
	...
	const repo = createRepoWithFiles(beforeFiles, afterFiles, 'unified-review-diff-shapes-e2e-');
	...
	for (const { name, scenario, anchorLabel } of cases) {
		...
		await waitForBuffer(terminal, (rows) => rows.some((row) => row.includes(marker)));

		await expect(terminal.getByText(scenario.labels[anchorLabel].text, { strict: false })).toBeVisible();
	}
});
