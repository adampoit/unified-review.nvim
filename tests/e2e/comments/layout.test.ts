...
async function collectDiffLineVisualRows(terminal: Terminal, targets: DiffLineTarget[]): Promise<DiffLineVisualRow[]> {
	const outputPath = join(mkdtempSync(join(tmpdir(), 'unified-review-visual-rows-')), 'rows.json');
...
test('right-side comments in later insertions do not move earlier replacement alignment', async ({ terminal }) => {
	const scenario = diffScenario([
		file('src/comment-alignment-regression.txt', [
			ctx('prefix', ['CTX_PREFIX_001', 'CTX_PREFIX_002', 'CTX_PREFIX_003', 'CTX_PREFIX_004']),
			del('old_block', [
				'OLD_WRAPPER_START',
	...
	await expect(terminal.getByText(rightAlignedLine, { strict: false })).toBeVisible();
...
test.describe('narrow terminal inline comment layout', () => {
	...
	test('wrapped comments preserve spacer rows in narrow panes', async ({ terminal }) => {
		const scenario = diffScenario([
			file('src/inline.lua', [
		...
		const body = 'NARROWBODY wraps enough text to exercise the narrower side by side panes';
		...
		const rows = await waitForBuffer(
			terminal,
			(visibleRows) =>
				visibleRows.some((row) => row.includes('NARROWBODY') && row.includes('╱')) &&
		...
		assert.ok(bodyRow, `expected visible wrapped body row in:\n${rows.join('\n')}`);
		const midpoint = Math.floor(bodyRow.length / 2);
		assert.ok(bodyRow.indexOf('NARROWBODY') >= midpoint, `expected NARROWBODY on the right side; got:\n${bodyRow}`);
		assert.ok(
...
test.skip('comments at the start of a file-level insertion keep the shared tail aligned', async ({ terminal }) => {
	const repo = createStartOfFileAdditionRepo();
	await openReviewForRepo(terminal, repo);
	await expect(terminal.getByText('ADDED_AT_FILE_START', { strict: false })).toBeVisible();
	...
	const rows = await waitForBuffer(
		terminal,
		(visibleRows) =>
			visibleRows.some((row) => row.includes(body)) && visibleRows.some((row) => occurrences(row, marker) >= 2),
	);
