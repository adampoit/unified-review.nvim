...
import { createRepoFromDiffScenario, delay, luaString, runLua, waitForBuffer } from '../helpers.js';

export { luaString, occurrences, openReviewForRepo, reopenReviewForRepo, runLua, waitForBuffer } from '../helpers.js';

function createInlineRepo(ops: Parameters<typeof file>[1]) {
	return createRepoFromDiffScenario(diffScenario([file('src/inline.lua', ops)]));
}
...
export function createReplacementInlineCommentRepo() {
	return createInlineRepo([
		...
		add('replacement_new', ['REPLACEMENT_NEW_VALUE', 'REPLACEMENT_NEW_EXTRA', 'REPLACEMENT_NEW_RETURN']),
		ctx('shared_after', [
...
export function createContextBeforeUnevenReplacementRepo() {
	return createInlineRepo([
		...
		ctx('schedule', ['SCHEDULE_SYNC_MARKER', 'SCHEDULE_SYNC_BODY', 'SCHEDULE_SYNC_END']),
	]);
}
...
export function createSummaryInsertionBeforeCopyRepo() {
	return createInlineRepo([
		...
		ctx('summary_copy', ['SUMMARY_COPY_MARKER', 'SUMMARY_COPY_BODY', 'SUMMARY_COPY_END']),
	]);
}
...
export function createContextBeforePureDeletionRepo() {
	return createInlineRepo([
		...
		del('pure_deletion', ['PURE_DELETION_001', 'PURE_DELETION_002', 'PURE_DELETION_003']),
		ctx('shared_after', ['SHARED_AFTER_CONTEXT_DELETION', 'SHARED_AFTER_BODY', 'SHARED_AFTER_END']),
	]);
}
...
export function createStartOfFileAdditionRepo() {
	return createInlineRepo([
		...
		ctx('shared_after', ['SHARED_AFTER_START_ADDITION', 'SHARED_AFTER_BODY', 'SHARED_AFTER_END']),
	]);
}
...
export function createSmallDeletedBlockRepo() {
	return createInlineRepo([
		ctx('before', ['SMALL_SHARED_BEFORE']),
		del('small_deleted', ['SMALL_DELETED_TARGET_001', 'SMALL_DELETED_TARGET_002 = SMALL_DELETED_TARGET_001']),
...
export function createLeftLongerReplacementRepo() {
	return createInlineRepo([
		...
		del('left_old', ['LEFT_LONGER_OLD_001', 'LEFT_LONGER_OLD_002', 'LEFT_LONGER_OLD_003']),
...
export function createSmallReplacementRepo() {
	return createInlineRepo([
		...
		del('small_old', ['SMALL_REPLACEMENT_OLD_001', 'SMALL_REPLACEMENT_OLD_002']),
		add('small_new', ['SMALL_REPLACEMENT_NEW_001', 'SMALL_REPLACEMENT_NEW_002', 'SMALL_REPLACEMENT_NEW_003']),
		ctx('shared_after', [
...
export function createAdjacentHunksRepo() {
	return createInlineRepo([
		...
		add('second_new', ['ADJACENT_SECOND_SHARED_NEW', 'ADJACENT_SECOND_ADDED_TARGET']),
		ctx('shared_after', ['SHARED_AFTER_ADJACENT_HUNKS', 'SHARED_AFTER_BODY', 'SHARED_AFTER_END']),
	]);
}
	...
	return createRepoFromDiffScenario(
		diffScenario([
			file('src/inline.lua', [
				...
				ctx('inline_shared', ['SHARED_AFTER_TWO_FILE_A', 'SHARED_AFTER_BODY', 'SHARED_AFTER_END']),
			]),
			file('src/other.lua', [
				del('other_old', ['OTHER_BEFORE', 'OTHER_BEFORE_RETURN']),
				add('other_new', ['OTHER_AFTER', 'OTHER_ADDED_TARGET', 'OTHER_AFTER_RETURN']),
			]),
		]),
	);
}
...
export async function createInlineComment(
	...
	await expect(terminal.getByText(visibleNeedle, { strict: false })).toBeVisible();
}

export function scrollDiffSideToLine(terminal: Terminal, side: 'left' | 'right', line: number) {
...
export async function assertInlineVisualRowsAligned(terminal: Terminal, body: string, side: 'left' | 'right') {
	const script = `
