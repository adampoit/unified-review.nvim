...
test('persisted inline comments remain aligned after closing and reopening the review', async ({ terminal }) => {
...
test('persisted range comments keep replies and resolved/exported state after reopening', async ({ terminal }) => {
	...
	captureTerminal(terminal, 'inline comments - persisted range reply resolved exported');
...
test('deleting a draft removes its inline block and restores downstream alignment', async ({ terminal }) => {
	const repo = createSmallAddedBlockRepo();
	await openReviewForRepo(terminal, repo);
	...
	await expect(terminal.getByText('INLINE_E2E_THREAD_COUNT_0', { strict: false })).toBeVisible();
	...
	assert.equal(occurrences(rows.join('\n'), body), 0, `expected deleted body to be absent; got:\n${rows.join('\n')}`);
	assert.ok(
		rows.some((row) => occurrences(row, marker) >= 2),
		`expected shared marker ${marker} on both sides of one row; got:\n${rows.join('\n')}`,
	);
...
test('inline comments remain aligned after switching files and returning', async ({ terminal }) => {
	const repo = createTwoFileRepo();
	await openReviewForRepo(terminal, repo);
	selectFileByPath(terminal, 'src/inline.lua');
	await expect(terminal.getByText('TWO_FILE_ADDED_TARGET', { strict: false })).toBeVisible();
	...
	await expect(terminal.getByText('OTHER_ADDED_TARGET', { strict: false })).toBeVisible();
	selectFileByPath(terminal, 'src/inline.lua');
	await expect(terminal.getByText(body, { strict: false })).toBeVisible();
...
test('resolved and exported thread markers preserve spacer alignment', async ({ terminal }) => {
	const repo = createAddedBlockInlineCommentRepo();
	await openReviewForRepo(terminal, repo);
	...
	assert.ok(bodyRow, `expected visible body row ${body}`);
	assert.equal(rows.filter((row) => row.includes(body) && row.includes('╱')).length, 1);
	const midpoint = Math.floor(bodyRow.length / 2);
	assert.ok(bodyRow.indexOf(body) >= midpoint, `got:\n${bodyRow}`);
	assert.ok(bodyRow.indexOf('╱') < midpoint, `got:\n${bodyRow}`);
	assert.ok(
