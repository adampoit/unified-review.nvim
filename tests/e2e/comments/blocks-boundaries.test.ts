...
test.describe('one-sided block comments', () => {
	test('right-side comments expose an opposite spacer on the same computed visual row', async ({ terminal }) => {
	...
	test('right-side comments inside added blocks get opposite-side slash rows', async ({ terminal }) => {
		...
		await openReviewForRepo(terminal, repo);
		await expect(terminal.getByText('ADDED_BLOCK_INSERTED_001', { strict: false })).toBeVisible();
		...
		captureTerminal(terminal, 'inline comments - added block paired spacer rows');
	...
	test('left-side comments inside deleted blocks get opposite-side slash rows', async ({ terminal }) => {
		const scenario = diffScenario([
			file('src/inline.lua', [
				ctx('prefix', 80),
				del('deleted_block', [
				...
				ctx('shared_after', ['SHARED_AFTER_DELETED_BLOCK', 'SHARED_AFTER_BODY', 'SHARED_AFTER_END']),
			]),
		]);
		const repo = createRepoFromDiffScenario(scenario);
		await openReviewForRepo(terminal, repo);
		await expect(terminal.getByText('DELETED_BLOCK_REMOVED_001', { strict: false })).toBeVisible();
		...
		captureTerminal(terminal, 'inline comments - deleted block paired spacer rows');
		...
		assert.ok(bodyRow.indexOf(body) < midpoint, `got:\n${bodyRow}`);
		assert.ok(
			Array.from(bodyRow.matchAll(/╱/g), (match) => match.index || 0).some((column) => column >= midpoint),
			`got:\n${bodyRow}`,
		);
		assert.ok(rows.some((row) => occurrences(row, marker) >= 2));
	});
...
test.describe('hunk-boundary comments', () => {
	test('right-side comments on the first added line', async ({ terminal }) => {
		const scenario = diffScenario([
			file('src/inline.lua', [
				...
				ctx('shared_after', ['SHARED_AFTER_SMALL_ADDED', 'SHARED_AFTER_BODY', 'SHARED_AFTER_END']),
			]),
		]);
		const repo = createRepoFromDiffScenario(scenario);
		await openReviewForRepo(terminal, repo);
	...
	test('left-side comments on the first deleted line', async ({ terminal }) => {
		const scenario = diffScenario([
			file('src/inline.lua', [
				...
				ctx('shared_after', ['SHARED_AFTER_SMALL_DELETED', 'SHARED_AFTER_BODY', 'SHARED_AFTER_END']),
			]),
		]);
		const repo = createRepoFromDiffScenario(scenario);
		await openReviewForRepo(terminal, repo);
	...
	test('right-side context comments immediately after additions', async ({ terminal }) => {
		const scenario = diffScenario([
			file('src/inline.lua', [
				ctx('before', ['SMALL_SHARED_BEFORE']),
				add('small_added', ['SMALL_ADDED_TARGET_001', 'SMALL_ADDED_TARGET_002']),
		...
		captureTerminal(terminal, 'inline comments - boundary right context after added');
	...
	test('left-side context comments immediately after deletions', async ({ terminal }) => {
		const scenario = diffScenario([
			file('src/inline.lua', [
				ctx('before', ['SMALL_SHARED_BEFORE']),
				del('small_deleted', [
		...
		captureTerminal(terminal, 'inline comments - boundary left context after deleted');
...
test('multiple comments at different offsets in one added block keep the shared tail aligned', async ({ terminal }) => {
	...
	const rows = await waitForBuffer(
		...
		(visibleRows) =>
			...
			visibleRows.some((row) => row.includes(secondBody) && row.includes('╱')) &&
			visibleRows.some((row) => occurrences(row, marker) >= 2),
	);
...
test('context-line comments before an uneven replacement get same-row slash fillers', async ({ terminal }) => {
	const repo = createContextBeforeUnevenReplacementRepo();
	await openReviewForRepo(terminal, repo);
	await expect(terminal.getByText('TAB_LABEL_CONTEXT', { strict: false })).toBeVisible();
	...
	captureTerminal(terminal, 'inline comments - context before uneven replacement');
...
test('comments inside large one-sided insertions preserve downstream side-by-side alignment', async ({ terminal }) => {
	const repo = createSummaryInsertionBeforeCopyRepo();
	await openReviewForRepo(terminal, repo);
	await expect(terminal.getByText('ADD_SUMMARY_INSERT_001', { strict: false })).toBeVisible();
...
test('context-line comments before a pure deletion splice into target-side codediff filler', async ({ terminal }) => {
	const repo = createContextBeforePureDeletionRepo();
	await openReviewForRepo(terminal, repo);
	await expect(terminal.getByText('CONTEXT_BEFORE_PURE_DELETION', { strict: false })).toBeVisible();
...
test.describe('file boundary comments', () => {
	test('comments inside end-of-file insertions get opposite slash rows', async ({ terminal }) => {
		const scenario = diffScenario([
			file('src/inline.lua', [
				ctx('prefix', 20),
				ctx('shared_before', ['SHARED_BEFORE_EOF_ADDITION', 'SHARED_AFTER_BODY', 'SHARED_AFTER_END']),
				add('eof_added', ['EOF_ADDED_COMMENT_TARGET', 'EOF_ADDED_RETURN']),
			]),
		]);
		const repo = createRepoFromDiffScenario(scenario);
		await openReviewForRepo(terminal, repo);
		await expect(terminal.getByText('EOF_ADDED_COMMENT_TARGET', { strict: false })).toBeVisible();
	...
	test('comments inside end-of-file deletions get opposite slash rows', async ({ terminal }) => {
		const scenario = diffScenario([
			file('src/inline.lua', [
				ctx('prefix', 20),
				ctx('shared_before', ['SHARED_BEFORE_EOF_DELETION', 'SHARED_AFTER_BODY', 'SHARED_AFTER_END']),
				del('eof_deleted', ['EOF_DELETED_COMMENT_TARGET', 'EOF_DELETED_RETURN']),
			]),
		]);
		const repo = createRepoFromDiffScenario(scenario);
		await openReviewForRepo(terminal, repo);
		await expect(terminal.getByText('EOF_DELETED_COMMENT_TARGET', { strict: false })).toBeVisible();
...
test('multiple threads and replies at the same target keep the shared tail aligned', async ({ terminal }) => {
	const repo = createAddedBlockInlineCommentRepo();
	await openReviewForRepo(terminal, repo);
	...
	runLua(terminal, `require('unified_review.session.manager').reply(nil, ${luaString(replyBody)})`);
	...
	captureTerminal(terminal, 'inline comments - same target threads and replies');
	{
		const bodyRow = rows.find((row) => row.includes(firstBody));
		assert.ok(bodyRow, `expected visible body row ${firstBody}`);
		const midpoint = Math.floor(bodyRow.length / 2);
...
test('long multi-paragraph comments that wrap keep the shared tail aligned', async ({ terminal }) => {
	const repo = createSmallAddedBlockRepo();
	await openReviewForRepo(terminal, repo);
	const body =
	...
	const rows = await waitForBuffer(
		terminal,
		(visibleRows) =>
			visibleRows.some((row) => row.includes('INLINE_E2E_LONG_BODY_START') && row.includes('╱')) &&
			visibleRows.some((row) => row.includes('INLINE_E2E_LONG_BODY_SECOND_PARAGRAPH') && row.includes('╱')) &&
			visibleRows.some((row) => occurrences(row, marker) >= 2),
	);
...
test('comments near adjacent hunks do not disturb later hunk alignment', async ({ terminal }) => {
	const repo = createAdjacentHunksRepo();
	await openReviewForRepo(terminal, repo);
	await expect(terminal.getByText('ADJACENT_FIRST_ADDED_TARGET', { strict: false })).toBeVisible();
