...
import { createInlineCommentThroughEditor, openReviewForRepo, runLua, waitForBuffer } from './helpers.js';

configureNvimTest(test, { columns: 180, rows: 40 });

test.describe('comment editor workflow inline rendering', () => {
	test('comment editor is anchored below its target with an opposite-side spacer', async ({ terminal }) => {
		...
		await expect(terminal.getByText('INLINE_EDITOR_TARGET', { strict: false })).toBeVisible();
		...
		const rows = await waitForBuffer(
			...
			(visibleRows) =>
				...
				visibleRows.some((row) => row.includes('Comment ·') && row.includes('╱')),
		);
		captureTerminal(terminal, 'inline comment editor - anchored to diff');

		const targetRow = rows.findIndex((row) => row.includes('INLINE_EDITOR_TARGET'));
	...
	test('saving from insert mode returns focus to the diff in normal mode', async ({ terminal }) => {
		...
		const repo = createRepoFromDiffScenario(scenario);
		await openReviewForRepo(terminal, repo);
		await expect(terminal.getByText('INSERT_MODE_TARGET', { strict: false })).toBeVisible();
	...
	test('right-side comments created through :UnifiedReview comment render inline with left spacers', async ({
		...
		assert.equal(occurrences(rows.join('\n'), body), 1, `expected exactly one visible inline body ${body}`);
