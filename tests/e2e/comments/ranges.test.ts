...
import { createInlineRangeComment, openReviewForRepo, waitForBuffer } from './helpers.js';

configureNvimTest(test, { columns: 180, rows: 40 });

test.describe('range comments', () => {
	test('right-side ranges inside added blocks preserve spacer alignment', async ({ terminal }) => {
		...
		assert.ok(bodyRow.indexOf(body) >= midpoint, `expected ${body} on the right side; got:\n${bodyRow}`);
		assert.ok(
	...
	test('left-side ranges inside deleted blocks preserve spacer alignment', async ({ terminal }) => {
		const scenario = diffScenario([
			file('src/inline.lua', [
				ctx('before', ['SMALL_SHARED_BEFORE']),
				del('small_deleted', ['SMALL_DELETED_TARGET_001', 'SMALL_DELETED_TARGET_002']),
		...
		assert.equal(occurrences(rows.join('\n'), body), 1);
		const midpoint = Math.floor(bodyRow.length / 2);
		assert.ok(bodyRow.indexOf(body) < midpoint, `expected ${body} on the left side; got:\n${bodyRow}`);
		assert.ok(
	...
	test('right-side ranges spanning context and changed lines preserve spacer alignment', async ({ terminal }) => {
		const scenario = diffScenario([
			file('src/inline.lua', [
	...
	test('left-side ranges spanning context and changed lines preserve spacer alignment', async ({ terminal }) => {
		const scenario = diffScenario([
			file('src/inline.lua', [
				ctx('before', ['SMALL_REPLACEMENT_BLOCK']),
				del('small_old', ['SMALL_REPLACEMENT_OLD_001', 'SMALL_REPLACEMENT_OLD_002']),
	...
	test('right-side ranges spanning adjacent hunks preserve later hunk alignment', async ({ terminal }) => {
		const scenario = diffScenario([
			file('src/inline.lua', [
				ctx('prefix', 12),
