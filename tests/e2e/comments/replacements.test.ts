...
import { configureNvimTest, createRepoFromDiffScenario, occurrences } from '../helpers.js';
import { createInlineComment, openReviewForRepo, waitForBuffer } from './helpers.js';

configureNvimTest(test, { columns: 180, rows: 40 });

test.describe('replacement comments', () => {
	test('right-side comments on replacement-only lines get opposite-side slash rows', async ({ terminal }) => {
		...
		await expect(terminal.getByText('REPLACEMENT_BLOCK', { strict: false })).toBeVisible();
		...
		captureTerminal(terminal, 'inline comments - right replacement target line');
	...
	test('left-side comments on replacement lines get opposite-side slash rows', async ({ terminal }) => {
		const scenario = diffScenario([
			file('src/inline.lua', [
				ctx('prefix', 120),
				ctx('replacement_block', ['REPLACEMENT_BLOCK']),
		...
		captureTerminal(terminal, 'inline comments - left replacement paired spacer rows');
	...
	test('right-side comments in equal-length replacements', async ({ terminal }) => {
		const scenario = diffScenario([
			file('src/inline.lua', [
	...
	test('left-side comments in equal-length replacements', async ({ terminal }) => {
		const scenario = diffScenario([
			file('src/inline.lua', [
				ctx('before', ['EQUAL_REPLACEMENT_BLOCK']),
				del('equal_old', ['EQUAL_OLD_001', 'EQUAL_OLD_002', 'EQUAL_OLD_003']),
	...
	test('left-side comments on left-only replacement rows', async ({ terminal }) => {
		const scenario = diffScenario([
			file('src/inline.lua', [
		...
		captureTerminal(terminal, 'inline comments - left longer replacement left only');
	...
	test('right-side comments in left-longer replacements', async ({ terminal }) => {
		const scenario = diffScenario([
			file('src/inline.lua', [
				ctx('before', ['LEFT_LONGER_REPLACEMENT_BLOCK']),
				del('left_old', ['LEFT_LONGER_OLD_001', 'LEFT_LONGER_OLD_002', 'LEFT_LONGER_OLD_003']),
		...
		captureTerminal(terminal, 'inline comments - left longer replacement right');
...
test('comments on both sides of the same replacement row preserve downstream alignment', async ({ terminal }) => {
	const scenario = diffScenario([
		file('src/inline.lua', [
			ctx('prefix', 120),
			ctx('replacement_block', ['REPLACEMENT_BLOCK']),
	...
	captureTerminal(terminal, 'inline comments - both sides same replacement row');
