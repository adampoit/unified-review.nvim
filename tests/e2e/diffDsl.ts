...
export function file(path: string, opsOrHunks: Array<Operation | HunkSpec>) {
	...
	return {
		path,
		hunks: hasHunks ? (opsOrHunks as HunkSpec[]) : [hunk(opsOrHunks as Operation[])],
	};
}
...
function opLines(operation: Operation): string[] {
	...
	return Array.from({ length: count }, (_, index) => generatedText(operation.kind, operation.label, index + 1));
}

function addGapLines(beforeLines: string[], afterLines: string[], count: number) {
...
function recordLabel(
	...
	labels[`${operation.label}:${index}`] = {
		...
		kind: operation.kind,
		text,
		side: operation.kind === 'deleted' ? 'left' : operation.kind === 'added' ? 'right' : 'both',
		oldLine,
		newLine,
	};
}
...
function buildFile(spec: FileSpec, labels: DiffScenario['labels']): BuiltFile {
	...
	const patchLines = [`diff --git a/${spec.path} b/${spec.path}`, `--- a/${spec.path}`, `+++ b/${spec.path}`];
	let additions = 0;
	let deletions = 0;
		...
		for (const operation of currentHunk.ops) {
			...
			lines.forEach((text, index) => {
				...
				if (operation.kind === 'context') {
					...
					recordLabel(labels, spec.path, operation, labelIndex, text, oldLine, newLine);
					oldLine += 1;
					newLine += 1;
				} else if (operation.kind === 'deleted') {
					beforeLines.push(text);
					patchLines.push(`-${text}`);
					recordLabel(labels, spec.path, operation, labelIndex, text, oldLine, undefined);
					oldLine += 1;
					deletions += 1;
				} else {
					afterLines.push(text);
					patchLines.push(`+${text}`);
					recordLabel(labels, spec.path, operation, labelIndex, text, undefined, newLine);
					newLine += 1;
					additions += 1;
				}
			});
...
export function diffScenario(files: FileSpec[]): DiffScenario {
	...
	const additions = builtFiles.reduce((sum, builtFile) => sum + builtFile.additions, 0);
	const deletions = builtFiles.reduce((sum, builtFile) => sum + builtFile.deletions, 0);

	return {
		path: builtFiles[0]?.path || '',
		patch: builtFiles.map((builtFile) => builtFile.patch.replace(/\n$/, '')).join('\n') + '\n',
		files: builtFiles,
		labels,
		additions,
		deletions,
