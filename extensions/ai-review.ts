...
import { buildContextInit, buildImportInit, buildOpenInit } from './lib/nvim-scripts.ts';
import {
	commandExists,
	createTempDir,
...
export function nvimExitSummary(result: NvimRunResult, diagnostics?: NvimDiagnostics): string {
	const details = [
		result.signal
...
export function registerAiReviewExtension(
	...
	pi.on('session_shutdown', async () => {
		...
		if (importedReviewToOpen?.tempDir !== pendingReview?.tempDir) removeReview(importedReviewToOpen);
		pendingReview = undefined;
		importedReviewToOpen = undefined;
	});

	pi.on('agent_settled', async (_event, ctx) => {
		if (promptingToOpenReview || !importedReviewToOpen || ctx.mode !== 'tui') return;
		...
		try {
			...
			if (openNow) {
				...
				dependencies.writeText(openInitPath, buildOpenInit(review.selection.target));
				const result = await dependencies.runInteractiveNvim(ctx, ['-S', openInitPath]);
				if (result.error || result.exitCode !== 0) {
					ctx.ui.notify(
						result.error ? `Failed to launch nvim: ${result.error}` : nvimExitSummary(result),
						'warning',
					);
				}
			}
			...
			if (review.keepTempDir) {
				ctx.ui.notify(`Import warnings retained in ${review.tempDir}`, 'warning');
			} else {
				removeReview(review);
			}
		}
	...
	pi.registerTool({
		...
		description: 'Submit structured unified-review agent feedback for the active /ai-review workflow.',
		promptSnippet: 'Submit unified-review.agent-feedback.v1 JSON after completing an /ai-review code review.',
		promptGuidelines: [
		...
		async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
			...
			if (!pending) {
				throw new Error('No active /ai-review workflow is waiting for feedback.');
			}
			...
			const review = {
				...
				source: {
					...
					model: params.source?.model ?? (ctx.model ? `${ctx.model.provider}/${ctx.model.id}` : undefined),
				},
			};
			dependencies.writeText(pending.feedbackPath, JSON.stringify(review, null, 2));

			const importInitPath = join(pending.tempDir, 'import-init.lua');
			dependencies.writeText(
				importInitPath,
				buildImportInit(pending.feedbackPath, pending.importDiagnosticsPath, pending.selection.target),
			);
			const nvimResult = await dependencies.runHeadlessNvim(pi, ctx.cwd, importInitPath);
			const diagnostics = dependencies.readJsonIfExists<ImportDiagnostics>(pending.importDiagnosticsPath);
			if (nvimResult.exitCode !== 0 || !diagnostics || diagnostics.status === 'error') {
				throw new Error(`Failed to import review feedback.\n${nvimExitSummary(nvimResult, diagnostics)}`);
			}
		...
		handler: async (_args, ctx) => {
			...
			try {
				...
				if (selectResult.error) {
					ctx.ui.notify(`Failed to launch nvim: ${selectResult.error}`, 'error');
					return;
				}
				...
				dependencies.writeText(
					contextInitPath,
					buildContextInit(contextPath, selectionValue.target, contextDiagnosticsPath),
				);
				const contextResult = await dependencies.runHeadlessNvim(pi, ctx.cwd, contextInitPath);
				const contextDiagnostics = dependencies.readJsonIfExists<NvimDiagnostics>(contextDiagnosticsPath);
				if (contextResult.exitCode !== 0 || contextDiagnostics?.status === 'error') {
					keepTempDir = true;
					ctx.ui.notify(
