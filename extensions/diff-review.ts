...
export function formatNvimExitDetails(
	...
	if (diagnostics?.message) details.push(`export message: ${diagnostics.message}`);
	if (diagnostics?.thread_count !== undefined) {
		details.push(`threads: ${diagnostics.exported_thread_count ?? '?'}/${diagnostics.thread_count} exported`);
	}
	...
	if (diagnostics?.modified_buffers?.length) {
		details.push(
			`modified buffers: ${diagnostics.modified_buffers
				.map((buf) => `${buf.name || '[No Name]'}${buf.buftype ? ` (${buf.buftype})` : ''}`)
				.join(', ')}`,
		);
	}
...
export function registerDiffReviewExtension(
	...
	pi.registerCommand('review', {
		description: 'Open Neovim to review a diff, then insert the exported review Markdown into the editor',
		handler: async (_args, ctx) => {
			...
			dependencies.writeText(initPath, buildReviewExportInit(reviewPath, diagnosticsPath));
			...
			try {
				...
				const diagnostics = dependencies.readJsonIfExists<NvimDiagnostics>(diagnosticsPath);
