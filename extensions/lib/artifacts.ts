...
export const AGENT_FEEDBACK_SCHEMA = 'unified-review.agent-feedback.v1' as const;
export const AGENT_SELECTION_SCHEMA = 'unified-review.agent-selection.v1' as const;
...
export const feedbackSchema = Type.Object({
	...
	comments: Type.Array(
		Type.Object({
			...
			author: Type.Optional(Type.String()),
			severity: Type.Optional(StringEnum(['error', 'warning', 'info', 'nit'] as const)),
...
export function isSelectionArtifact(value: unknown): value is SelectionArtifact {
	if (!value || typeof value !== 'object') return false;
	const artifact = value as Partial<SelectionArtifact>;
	return artifact.schema === AGENT_SELECTION_SCHEMA && artifact.target !== undefined;
}

export function isContextArtifact(value: unknown): value is ContextArtifact {
	if (!value || typeof value !== 'object') return false;
	const artifact = value as Partial<ContextArtifact>;
	return artifact.schema === AGENT_CONTEXT_SCHEMA && Array.isArray(artifact.files);
}
